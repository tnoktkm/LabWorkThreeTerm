...
class shop():
	...
	def clear(self):
		for item in self.getOrderCodes():
			self.removeOrder(item)
		for item in self.getOrdererCodes():
			self.removeOrderer(item)
		for item in self.getGoodCodes():
			self.removeGood(item)
	...
	#good----------------
	def newGood(self, code=0, price='',dilivery='',description=''):self.__good.newRec(code,price,dilivery,description)
	...
	#order----------------------------------
	def newOrder(self, code=0, orderer=None, good=None, amount='', data=''):self.__order.newRec(code,orderer,good,amount,data)
	def findOrderByCode(self, code):return self.__order.findByCode(code)
	def getOrderCodes(self):return self.__order.getCodes()

	def getOrderOrdererCode(self, code): return self.__order.findByCode(code).getCode()
	def getOrderGoodCode(self, code): return self.__order.findByCode(code).getCode()
